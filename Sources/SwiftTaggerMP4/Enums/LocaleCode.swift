...
enum LocaleCode: String, CaseIterable {
    ...
    private static let unCodeToCodeMapping: [Int: LocaleCode]? = {
        var mapping: [Int: LocaleCode] = [:]
        for code in LocaleCode.allCases {
            if let unCode = code.unCode {
                mapping[unCode] = code
            } else {
                return nil
            }
    ...
    var associatedLanguages: [ISO6392Code]? {
        switch self {
